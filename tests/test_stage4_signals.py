...
def test_batch36_stage4_csv_validation_and_unknown_partner(
    ...
    Migrated to `scored_workspace` so stages 1+2 (needed to populate
    the partner registry the unknown-partner check compares against)
    run ONCE per pytest session instead of per-test -- saves ~12s
    off the slowest single test in the suite.
    """
    ws_dst = scored_workspace
    ...
    # Snapshot the pre-existing unknown_partner_in_csv error count;
    # the assertion below only cares that THIS run added one. Using
    # a session-cached workspace means earlier (different) tests
    # may have left run_errors rows behind via their own pipeline
    # runs, so we measure the delta instead of the absolute count.
    ...
    c.close()
    assert n_unknown_errors > pre_count, (
        f"expected an additional unknown_partner_in_csv run_error; "
        f"pre={pre_count} post={n_unknown_errors}"
    )
